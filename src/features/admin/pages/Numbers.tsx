      ...
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <SyncTwilioButton onOrphansDetected={orphaned => {
            setOrphanedDialog({
              open: true,
              provider: 'twilio',
            ...
            <SyncVonageButton onOrphansDetected={orphaned => {
            setOrphanedDialog({
              open: true,
              provider: 'vonage',
              orphanedNumbers: orphaned
        ...
        {!loadingCredentials && !hasCredentials && <Alert variant="destructive">
        ...
        <PhoneNumberDialog open={dialogOpen} onOpenChange={setDialogOpen} />

        {orphanedDialog && <OrphanedNumbersDialog open={orphanedDialog.open} onOpenChange={open => !open && setOrphanedDialog(null)} provider={orphanedDialog.provider} orphanedNumbers={orphanedDialog.orphanedNumbers} onCleanup={handleCleanupOrphans} isLoading={deleteMutation.isPending} />}
      </div>
    </div>;
};
